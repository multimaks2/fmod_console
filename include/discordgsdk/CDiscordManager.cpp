...
#include "CDiscordManager.h"
    ...
    #define DISCORD_CLIENT_ID 769051765156347924
#endif
...
void CDiscordManager::UpdateActivity(SDiscordActivity& activity, std::function<void(EDiscordRes)> callback)
{
    ...
    m_StoredActivity.GetAssets().SetLargeText(activity.large_txt);
    ...
    m_StoredActivity.GetAssets().SetSmallText(activity.small_txt);
    m_StoredActivity.GetAssets().SetSmallImage(activity.small_image);
...
void CDiscordManager::RegisterPlay(bool connected)
{
    ...
    time(&currentTime);
    std::lock_guard<std::mutex> guardian(m_ThreadSafety);
    m_StoredActivity.SetDetails("Получение имени сервера...");
    m_StoredActivity.GetAssets().SetSmallText(SString("Подключен к %s:%i", *ipaddr, port));
     // TODO: возможно, свяжитесь с серверами MTA: SA и проверьте, является ли этот IP-адрес премиальным
     // содержащее маленькое изображение для установки с помощью этой функции
